// Item create/edit form. Renders core fields + dynamic sections from FieldDefs,
// with NFA-style collapse, a photo uploader (deferred upload on create), tag picker,
// and an unsaved-changes guard. See DESIGN §6.
import { useCallback, useMemo, useRef, useState } from 'react';
import { Link, useNavigate, useParams } from 'react-router-dom';
import { Icon } from '../components/Icon';
import { Field } from '../components/FieldInput';
import { TagPicker } from '../components/TagPicker';
import { ConfirmDialog, LoadingBlock, ErrorBlock, Spinner } from '../components/ui';
...
export function ItemFormPage({ mode }: { mode: 'create' | 'edit' }) {
  ...
  const cid = Number(params.collectionId);
  return <CreateLoader collectionId={cid} navigate={navigate} />;
}
...
function EditLoader({ itemId }: { itemId: number }) {
  ...
  return <FormBody mode="edit" collection={collection} item={item} navigate={navigate} />;
}
...
function FormBody({
  ...
  // Upload one queued photo to a known item id.
  ...
  const doSave = useCallback(
    async (opts: { addAnother: boolean }) => {
      ...
      try {
        ...
        if (mode === 'create') {
          ...
          setDirty(false);
          if (opts.addAnother) {
            // Reset to a blank same-collection form; stay on the create route.
            setCore(emptyCore());
            setFields({});
            setTags([]);
          ...
          setDirty(false);
          if (!opts.addAnother) navigate(`/items/${updated.id}`);
    ...
    [core.name, buildPayload, mode, createItem, updateItem, item, uploadAllQueued, queued, navigate, toast],
  );
  ...
  const title =
    mode === 'create' ? `New ${collection.name}` : `Edit ${item?.name ?? 'item'}`;
